...
def get_assessment_config(
    ...
    # Get tutor name (or "Self-Assessment" for self-initiated diagnostics)
    tutor_name = "Self-Assessment"
    if invite.tutor_id:
        # Self-serve diagnostic: tutor_id == student_id (or student_id not yet set but title is Diagnostic)
        is_self_serve = (
            invite.student_id is not None and str(invite.tutor_id) == str(invite.student_id)
        ) or (invite.title == "Diagnostic Assessment")
...
def start_assessment(
    ...
    if current_user:
        ...
        # Skip if the invite tutor is the student themselves (self-serve diagnostic)
        if current_user.tutor_id is None and invite.tutor_id and invite.tutor_id != current_user.id:
...
@router.get("/{token}/full-results")
def get_full_results(
    token: str,
    db: Session = Depends(get_db),
):
    """
    Unified results endpoint returning everything needed for the results page:
    section scores, domain heatmap, worst skills with lesson info, question review.
    """
    invite = db.query(Invite).filter(Invite.token == token).first()
    if not invite or not invite.test_session_id:
    ...
    if session.status != TestStatus.COMPLETED:
        raise HTTPException(status_code=400, detail="Assessment has not been completed yet")

    # --- IRT-based results (sections, domains, composite score) ---
    results = calculate_intake_results(db, session.id)
    if "error" in results:
        raise HTTPException(status_code=500, detail=results["error"])

    # --- Question review (flat list with skill/domain info) ---
    test_questions = db.query(TestQuestion).filter(
        TestQuestion.test_session_id == session.id
    ).order_by(TestQuestion.question_order).all()
    ...
    # Build skill accuracy map for worst_skills calculation
    skill_stats: dict = {}  # skill_id -> {correct, total, skill_name, domain_code}
    ...
    # --- Worst skills (bottom 5 by accuracy, min 1 question) ---
    # Build lesson lookup by skill_id
